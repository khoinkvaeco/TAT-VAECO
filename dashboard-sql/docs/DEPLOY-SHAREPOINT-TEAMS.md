...
| E4 | Thẻ hiện trong Teams nhưng **không có file** | `REPORT_EXPORT_DIR` chưa đặt/sai đường dẫn, hoặc tài khoản chạy service không có quyền ghi. → Xem kết quả nút Gửi ngay: `Files: không bật` = chưa đặt `REPORT_EXPORT_DIR`; `không ghi được file` = sai đường dẫn/thiếu quyền. Sửa cho khớp File Explorer; đảm bảo service chạy bằng đúng user đã đăng nhập OneDrive. |
