...
### Thuật ngữ thống nhất cho tab LGC

Bốn từ dưới đây dùng **nguyên một nghĩa** ở mọi nơi — nhãn KPI, tên cột, tooltip, biểu đồ,
README. Trước đây cùng một thứ bị gọi bằng nhiều tên (*thực xuất* / *bình thường* / *dòng xuất*)
nên đọc số rất dễ hiểu nhầm.

| Từ | Nghĩa |
|---|---|
| **Phiếu xuất** | Item ra khỏi kho bình thường (`QTY_CANCELED = 0`) |
| **Cancel** | Hủy **trước khi** hàng ra khỏi kho — `PICKSLIP_TEXT` kết thúc `…cancel` |
| **Return** | Hàng **đã ra kho rồi quay về** — `PICKSLIP_TEXT` kết thúc `…return` |
| **Receive** | Nhập kho (tab *Receiving*, `HISTORY` `VM = 'B1'`) |
| **Item** | **Đơn vị đếm** của cả hai tab — một dòng `PICKSLIP_BOOKED` / `HISTORY`. Một **phiếu** có nhiều **item**. |

*(“Dòng” đã bỏ khỏi giao diện: nó là từ của bảng dữ liệu, không phải từ của nghiệp vụ, và dễ
lẫn với “phiếu”.)*
...
### `WO_PART_ON_OFF.STATUS` — tình trạng BOOKING của lần thay thiết bị

Tài liệu AMOS: *“Booking status of the component change”*.

| Giá trị | Nghĩa | Hiện trên giao diện |
|---|---|---|
| `0` | Not Booked | **Chưa booking** (vàng) |
| `1` | Booked | **Đã booking** (xanh) |
| khác | — | hiện **nguyên văn**, chữ xám |

Giao diện hiện **chữ chứ không hiện số**: `0`/`1` trần trụi thì người đọc phải nhớ nghĩa, và rất
dễ lẫn với các cột `STATUS` khác của AMOS (pickslip dùng `1`/`11` cho việc hoàn toàn khác).
Giá trị lạ **không** bị âm thầm coi là “chưa booking” — hiện nguyên văn để lộ ra ngay.

Dùng ở: tab **Tra cứu Part On/Off**, báo cáo **Chỉ lắp / Chỉ tháo** (đặt ngay cạnh cột *Có on_off*),
và `GET /api/admin/diag/higher` → `bookingStatus` (phân bố `STATUS` trong kỳ, tách theo dòng
một phía / hai phía).
...
- **Ngày giờ xuất kho = `PICKSLIP_HEADER.PICKSLIP_DATE` + `PICKSLIP_HEADER.BOOKING_TIME`**
  (nghiệp vụ chỉ định). `BOOKING_TIME` là **giờ lập phiếu thật**, nên mọi item xuất kho đều có
  **đủ ngày giờ** — không còn nhóm “chỉ biết ngày”.
  *(Trước đây phải dò xem `MUTATION` của header có rơi đúng vào `PICKSLIP_DATE` không rồi mới dám
  lấy `MUTATION_TIME`; cách đó bỏ trống giờ của rất nhiều dòng. Đã bỏ.)*

  ⚠️ **`BOOKING_TIME` tính bằng PHÚT kể từ `00:00`** — tài liệu AMOS: *“Time of confirmation /
  booking in minutes since midnight”*. Cùng một AMOS dùng **ba** đơn vị thời gian khác nhau,
  rất dễ lẫn:

  | Cột | Đơn vị |
  |---|---|
  | `PICKSLIP_HEADER.BOOKING_TIME` | **phút** kể từ 00:00 |
  | `MUTATION_TIME` (`HISTORY`, `on_off`…) | **mili giây** kể từ 00:00 |
  | `WO_TEXT_ACTION.ACTION_TIME` (trang `/wp`, §7j) | **phút** kể từ 00:00 |

  Sai đơn vị thì giờ xuất kho vẫn hiện ra đẹp đẽ nhưng sai hoàn toàn, **không có gì báo lỗi**.
  Nên truy vấn vẫn đo `MAX(BOOKING_TIME)` (trên bảng tạm `#raw`, đã ở local — không tốn thêm
  lượt hỏi AMOS nào) và **cảnh báo ra nhật ký tiến trình** nếu giá trị ≥ 1440, tức không còn
  là phút kể từ nửa đêm nữa.
...
- **MỘT cột TAT duy nhất: `TAT return (ngày)`.** Cả hai đầu mốc đều có đủ ngày giờ
  (xuất kho = `PICKSLIP_DATE + BOOKING_TIME`; về kho = `HISTORY.MUTATION + MUTATION_TIME`),
  nên TAT được tính từ **mốc thật** rồi **quy ra ngày** — là số lẻ (`0.17 ngày ≈ 4 giờ`),
  không làm tròn ngày. Tooltip của ô hiện số giờ tương ứng.

  *(Trước đây có **hai** cột cho cùng một việc: `TAT hoàn kho (giờ)` tính theo giờ và `TAT return`
  tính ngày tròn — hai con số lệch nhau, người đọc phải đoán tin cột nào. Và KPI “Chính xác đến giờ”
  tồn tại chỉ để nói *“bao nhiêu dòng thực sự có giờ ở cả hai đầu”* — một cách thú nhận rằng phần
  còn lại đang lùi về ngày. Nay mọi mốc đều có giờ thật nên cả cột thừa lẫn KPI đó **đã bỏ**.)*
