...
| Chỉ số | Định nghĩa |
|--------|-----------|
| **TAT Trung tâm (department)** | Từ lúc **xuất kho** (`kho_ser1` `vm='T'`, voucher `P-...`) đến lúc **trả unservice** (`real_us1.del_time`). Tính theo **ngày**. Link `kho_ser1(partno,serialno,voucherno)`=`real_us1(partno,serialno,voucher_s)`. **Trung tâm**: `action_per` bắt đầu bằng `PA` → `PA`; ngược lại tra **`SIGN`** theo `action_per` (**nguồn chuẩn** — bảng nhân sự, đơn vị hiện tại); `SIGN` không có người đó mới dùng `real_us1.department`; cuối cùng mặc định `PA`. (Bảng không có `department` như kho_ser1/on_off thì dùng `mutator`.) *Trước đây `real_us1.department` được ưu tiên hơn `SIGN` — trái với thiết kế đã ghi — khiến một trung tâm miền Nam hiện ra dưới station HAN. Đo thực tế 1 tháng: 2.438 dòng đã đối ứng thì **51 dòng (2,1%)** có hai nguồn lệch nhau; kiểm tra bằng `/api/admin/diag/dept`.* |
