...
- **Đối ứng thủ công (label lệch):** tab riêng cho các ca bất thường — *tháo thiết bị này xuống, lắp thiết bị khác lên* nên **label của 2 thiết bị khác nhau**, join theo `labelno` không tự đối ứng được. Hệ thống **gợi ý cặp** theo 3 nguồn ưu tiên dần: (1) `WO_PART_ON_OFF` — một dòng ghi cả thiết bị lắp (`PARTNO/SERIALNO`) và thiết bị tháo (`PARTNO_OFF/SERIALNO_OFF`), độ tin cậy **Cao**; (2) `on_off` cùng `orderno`/`psn` — **Trung bình**; (3) `on_off` cùng số tàu, gần thời gian — **Thấp**; (4) **bảng Other (`on_ac`)** — thiết bị đã trả US nhưng chưa có phiếu xuất (đặc biệt mã **ROB** = tháo xuống trước), ghép theo **cùng Part No** + gần thời gian (cửa sổ −30/+60 ngày vì ROB có thể trả *trước* ngày lập phiếu), ưu tiên **cùng số tàu** → **Trung bình** nếu trùng tàu, **Thấp** nếu không. Cột *Cách ghép* ghi rõ mã lý do (vd `Other — ROB (Robbery…)`). Mỗi dòng ghi rõ **cách ghép + độ tin cậy**; bấm **✔ Xác nhận** → cặp lưu vào `data/reconcile-manual.json`, phiếu xuất hết nằm trong *Chưa đối ứng* và được tính là **đã đối ứng** trong KPI (`countManualPaired`). Xác nhận chỉ làm được từ **IP quản trị**.
