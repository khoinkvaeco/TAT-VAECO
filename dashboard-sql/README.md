...
- **Ngày giờ xuất kho = `PICKSLIP_HEADER.PICKSLIP_DATE` + `PICKSLIP_HEADER.BOOKING_TIME`**
  ...
  *(Trước đây phải dò xem `MUTATION` của header có rơi đúng vào `PICKSLIP_DATE` không rồi mới dám
  ...
  ⚠️ **Đơn vị của `BOOKING_TIME` không được ghi ở đâu cả**, mà cùng một AMOS đã dùng hai đơn vị
  khác nhau: `MUTATION_TIME` là **mili giây**, còn `WO_TEXT_ACTION.ACTION_TIME` (§7j) lại là **phút**.
  Nên chương trình **không đoán**: nó đo giá trị **lớn nhất trong chính lô dữ liệu vừa kéo về**
  (`< 1440` → phút · `< 86400` → giây · còn lại → mili giây), rồi **báo đơn vị đã chọn** ra
  nhật ký tiến trình và trường `bookingTimeDonVi` của API. Đoán sai đơn vị thì giờ vẫn hiện ra
  đẹp đẽ nhưng sai hoàn toàn — nên con số này phải nhìn thấy được.
