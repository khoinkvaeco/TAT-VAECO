...
| **Tháo chưa trả US** | `on_off vm='YA'` không có `real_us1` (link `historyno_`). Có thêm cột **PSN** và **Vị trí hiện tại** (`ROTABLES.location`) — cho biết thiết bị đã tháo xuống hiện đang nằm ở đâu. `ROTABLES` **vốn đã được join sẵn** trong truy vấn này (đang dùng ở `WHERE` cho `MUTATION`/`condition`) nên lấy thêm 2 cột **không tốn thêm lượt hỏi linked server** nào. |
