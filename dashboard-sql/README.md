...
- **Trợ lý TAT (chatbox nội bộ):** nút 💬 góc dưới-phải. Xử lý **cục bộ** theo luật/ý định (rule/intent) — **không gọi dịch vụ ngoài, không gửi dữ liệu ra ngoài**. Trả lời 5 nhóm: (1) **số liệu** ("TAT install tháng 7 của CNBDNT", "bao nhiêu thiết bị chưa đối ứng", "trung tâm nào TAT cao nhất"); (2) **tìm đối ứng** của 1 thiết bị ("đối ứng của serial 43842") → kiểm tra từng phiếu xuất gần nhất đã trả unservice / trả service (recertify) / hoàn kho / bị hủy hay CHƯA đối ứng; (3) **tra cứu 1 thiết bị** theo part/serial/label → lịch sử booking (on_off); (4) **định nghĩa nghiệp vụ** (TAT install/US return/hoàn kho, đối ứng, trả service, costcenter…); (5) **hướng dẫn dùng dashboard**. Câu hỏi không nêu kỳ/trung tâm sẽ dùng bộ lọc đang chọn trên trang. Logic NLU ở `chatbot.js` (hàm thuần, test được không cần DB).
  - **Không tự học (theo thiết kế):** bot chạy offline, không dùng LLM nên **không tự học/tự cập nhật kiến thức**. Thay vào đó, câu hỏi bot chưa hiểu được ghi vào `logs/chat-unknown-YYYY-MM-DD.log` để review; mở rộng kho tri thức bằng cách thêm mục vào `DEFINITIONS` / `USAGE` / `METRICS` trong `chatbot.js` (không cần đổi giao diện).
