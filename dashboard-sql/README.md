...
- **Điều hướng — 4 mục chính:** *📊 Tổng quan* · *📋 Các Báo cáo khác* (9 báo cáo) · *🏬 Kho / Chứng từ* · *🔎 Tra cứu Part On/Off*.
  Ba việc **của nhân viên kho** (*Quản lý xuất kho*, *Receiving*, *Repair Admin*) gom vào **một nhóm “Kho / Chứng từ”** có tab con — trước đây nằm rải rác 2 mục menu chính + 1 mục chôn ở cuối danh sách báo cáo. Nhân viên kho vào thẳng bằng **`/kho`** (xem mục dưới); các đơn vị khác không phải nhìn thấy nhóm này khi làm việc với TAT.
...
- **Trang riêng cho kho — `/kho`** (hoặc `/kho.html`): **cùng một `index.html`, cùng `script.js`, cùng service** — chỉ khác điểm vào. Frontend thấy đường dẫn `/kho` thì mở thẳng nhóm *Kho / Chứng từ*, ẩn các tab TAT, đổi tiêu đề trang, và **không chạy truy vấn dashboard** (nặng, mà tab đó đang ẩn). Có link *“Xem dashboard TAT đầy đủ →”* để quay lại. ⚠️ Đây là **đơn giản hóa giao diện, KHÔNG phải phân quyền** — ai gõ `/` vẫn xem được đầy đủ, đúng như hiện nay (mọi đơn vị đều được xem). Muốn **chặn** thật thì phải chặn ở server như `adminGuard`.
...
Hai tab **Quản lý xuất kho** và **Receiving** đối chiếu từng phiếu với **file PDF đã scan**
