...
| Chỉ số | Định nghĩa |
|--------|-----------|
| **TAT Trung tâm (department)** | Từ lúc **xuất kho** (`kho_ser1` `vm='T'`, voucher `P-...`) đến lúc **trả unservice** (`real_us1.del_time`). Tính theo **ngày**. Link `kho_ser1(partno,serialno,voucherno)`=`real_us1(partno,serialno,voucher_s)`. **Trung tâm** = **người lập phiếu xuất** (`kho_ser1.created_b2`) tra qua **`SIGN`**: mã bắt đầu bằng `PA` → `PA`; ngược lại tra `SIGN`; cuối cùng mặc định `PA`. **Mọi báo cáo dựa trên phiếu xuất đều dùng chung quy tắc này** (đã đối ứng, chưa đối ứng, trả service, xuất kho chưa lắp, TAT hoàn kho) → **một phiếu xuất luôn thuộc đúng một Trung tâm**, dù đã trả hay chưa. Bảng *Chi tiết TAT* có thêm cột **Người trả US** (`real_us1.action_per`) chỉ để đối chiếu. Các báo cáo **không có phiếu xuất** (TAT CUVT, Trả unservice, Other) vẫn lấy theo `SIGN(action_per)` → `real_us1.department` → `PA`, vì không có người lập phiếu để quy chiếu. Kiểm tra bằng `/api/admin/diag/dept` (có cả cột `dept_theo_nguoi_tra_US` là cách cũ để đối chiếu). |
...
| **Repair Admin** | Tồn đọng tại các **vị trí Unserviceable** (`[DWH_DB]..[STG_AMOS].[LOCATION].location_type = -4`), đếm theo **Station** và **Vị trí**, tách **< 30 ngày** / **≥ 30 ngày** — bố cục giống pivot Excel, có dòng **TỔNG CỘNG** và nút **⬇ Excel**. Hai nguồn: `LOCATION × ROTABLES` (có `orderno`/`orderdate`) và `LOCATION × CONSUMABLES` (có `owner`/`batchno`/`qty`). Đây là **ảnh chụp hiện trạng** nên **không** phụ thuộc kỳ báo cáo — chỉ lọc theo Station/Store đang chọn (điều kiện so sánh trực tiếp `l.[station] = @p`, không bọc `RTRIM`, để đẩy được xuống linked server). **Tuổi tồn đọng** tính từ `ROTABLES.orderdate`; giá trị được đổi sang ngày theo **kiểu thật driver trả về** (`Date` → dùng luôn; số → số ngày kể từ `AMOS_DATE_EPOCH`) và loại kết quả ngoài khoảng 1990–2100. ⚠️ Câu SQL cho `CONSUMABLES` **không có cột ngày** nên vật tư tiêu hao nằm ở cột **“Không rõ ngày”** — cần bổ sung tên cột ngày để đưa chúng vào 2 nhóm tuổi. Tab **Repair Admin (chi tiết)** liệt kê từng item kèm tuổi tồn đọng. |
...
| `GET /api/reports/:name` | Báo cáo (`issued-not-installed`, `removed-not-returned`, `not-reconciled`, `removed-before-installed`, `other`, `return-store-tat`, `repair-admin`, `repair-admin-detail`) |
