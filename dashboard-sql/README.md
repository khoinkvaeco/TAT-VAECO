...
| **Tháo chưa trả US** | `on_off vm='YA'` không có `real_us1` (link `historyno_`). Cột **Store** và **Location** là **vị trí hiện tại** của thiết bị: `ROTABLES` nối qua `psn`, rồi `LOCATION` nối theo `locationno_i` (`LEFT JOIN` nên không thể làm mất dòng; thiếu dữ liệu thì Store lùi về `on_off.store`). ⚠️ **Số dòng của báo cáo này giảm dần theo thời gian dù dữ liệu lịch sử không đổi**: `WHERE` còn lọc `RO.condition = 'US'` và `RO.MUTATION > @fromDay` — đây là **trạng thái hiện tại** của thiết bị, không phải trạng thái lúc tháo. Thiết bị đã được nhận/xử lý thì `condition` khác `US` và **biến mất khỏi báo cáo, kể cả khi xem lại kỳ cũ**. Dùng **`GET /api/admin/diag/rnr`** để xem từng điều kiện cắt bớt bao nhiêu dòng (6 bước cộng dồn) và phân bố `condition` hiện tại của các dòng bị loại. |
