...
| **Tháo chưa trả US** | `on_off vm='YA'` không có `real_us1` (link `historyno_`). Cột **Store** và **Location** là **vị trí hiện tại** của thiết bị: `ROTABLES` nối qua `psn`, rồi `LOCATION` nối theo `locationno_i` (`LEFT JOIN` nên không thể làm mất dòng; thiếu dữ liệu thì Store lùi về `on_off.store`). **Báo cáo tự cập nhật theo thực tế (đúng ý đồ):** điều kiện *chưa trả US* được xét tại **thời điểm xem**, nên thiết bị tháo tháng 7 mà trả US sang tháng 8 sẽ **tự biến mất** khỏi báo cáo tháng 7. Cùng một kỳ xem lại lúc khác ra số khác là bình thường. ⚠️ Nhưng `WHERE` còn hai điều kiện **khác tiêu chí**: `RO.condition = 'US'` và `RO.MUTATION > @fromDay` — chúng xét trạng thái hiện tại của rotable chứ không phải *đã trả US hay chưa*, nên có thể loại **oan** thiết bị thực sự chưa trả. Ngoài ra phép chống-nối dùng `labelno` thay vì `historyno_` (khóa mà docstring và `qTatDepartments` đều dùng) nên một lần trả US **khác** của cùng label cũng làm mất dòng. Cả hai được đo bằng `diag/rnr` (bước **2c** và bước **5→6**). Dùng **`GET /api/admin/diag/rnr`** để xem từng điều kiện cắt bớt bao nhiêu dòng (6 bước cộng dồn) và phân bố `condition` hiện tại của các dòng bị loại. |
